...
namespace HoneymoonshopTests.Tests
{
    public class ProductsTest
    {
        ...
        public ProductsController InitProducts(Mock<ApplicationDbContext> context)
        {
            ...
            return new ProductsController(context.Object);
        }
        ...
        public void ShouldCreateProduct()
        {
            ...
            var controller = InitProducts(mockDbContext);
            
            var newProduct = new Product() { artikelnummer = 0001, geslacht = "bruid", categorieId = 1, merkId = 2, omschrijving = "Mooie jurk", prijs = 600 };

            string[] colors = { "2" };
            string[] attributes = { "3" };

            var result = controller.Create(newProduct, colors, attributes);

            Assert.True(controller.ModelState.IsValid);

            mockDbContext.Verify(x => x.SaveChangesAsync(new System.Threading.CancellationToken()), Times.Once);

            mockDbContext.Verify(x => x.Add(It.IsAny<Product>()), Times.Once());

            mockDbContext.Verify(x => x.Add(It.Is<Product>(p => p.artikelnummer == newProduct.artikelnummer && p.artikelnummer == newProduct.artikelnummer && p.categorieId == newProduct.categorieId && p.geslacht == newProduct.geslacht && p.merkId == newProduct.merkId && p.omschrijving == newProduct.omschrijving && p.prijs == newProduct.prijs)), Times.Once());
        ...
        public void ShouldNotCreateProduct()
        {
            ...
            var newProduct = new Product() { artikelnummer = 0001, geslacht = "bruid", categorieId = 1, merkId = 2, omschrijving = "Mooie jurk"};

            string[] colors = { "2" };
            string[] attributes = { "3" };

            controller.ModelState.AddModelError("Error", "Geen Prijs");

            var result = controller.Create(newProduct, colors, attributes);

            var viewResult = Assert.IsType<ViewResult>(result);
            var model = (IQueryable<Product>)viewResult.Model;
